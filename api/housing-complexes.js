module.exports = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ complexes: features.map((feature) => { const p = feature.properties || {}; return { pnu: p.pnu || '', name: p.aphus_nm || '', averagePrice: Number(p.avrg_pblntf_pc || 0), units: Number(p.calc_aphus_ho_co || 0) }; }).filter((x) => x.pnu) });
  } catch { return res.status(502).json({ error: 'Could not reach official housing-price map API.' }); }
};
