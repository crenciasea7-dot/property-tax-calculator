...
const getJson = (url) => new Promise((resolve, reject) => {
  http.get(url, (upstream) => { let text = ''; upstream.setEncoding('utf8'); upstream.on('data', (chunk) => { text += chunk; }); upstream.on('end', () => { try { resolve({ status: upstream.statusCode || 500, body: JSON.parse(text) }); } catch { reject(new Error('Invalid JSON from VWorld')); } }); }).on('error', reject);
});
module.exports = async (req, res) => {
  ...
  try {
    const upstream = await getJson('http://api.vworld.kr/ned/data/getApartHousingPriceAttr?' + params); const body = upstream.body;
