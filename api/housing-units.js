...
const getJson = (url) => new Promise((resolve, reject) => {
  http.get(url, (upstream) => { let text = ''; upstream.setEncoding('utf8'); upstream.on('data', (chunk) => { text += chunk; }); upstream.on('end', () => { resolve({ status: upstream.statusCode || 500, text }); }); }).on('error', reject);
});
module.exports = async (req, res) => {
  const pnu = String(req.query?.pnu || '').replace(/\D/g, '');
  const dongNm = String(req.query?.dong || '').trim(); const hoNm = String(req.query?.ho || '').trim(); const key = process.env.VWORLD_API_KEY;
  ...
  if (dongNm) params.set('dongNm', dongNm); if (hoNm) params.set('hoNm', hoNm);
  try {
    const upstream = await getJson('http://api.vworld.kr/ned/data/getApartHousingPriceAttr?' + params); let body; try { body = JSON.parse(upstream.text); } catch { return res.status(502).json({ error: 'Official API did not return JSON.', detail: upstream.text.slice(0, 500) }); }
  ...
  } catch (error) { return res.status(502).json({ error: 'Could not reach official housing-price API.', detail: String(error.message || error) }); }
};
